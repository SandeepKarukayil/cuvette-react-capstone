...
function Info() {
  ...
  return (
    ...
    }}>{selectedMovies.map((movie,index)=><p key={index}>{movie.movie}</p>)}</div>:"No selected movies"}
    </div>
    <textarea style={{
      maxHeight:'400px',
      minHeight:'400px',
      ...
      borderRadius:'10px',
      marginTop:'20px',
    }} value={notes} onChange={updateNotes}></textarea>
    {news?<div><img src={news.urlToImage} width={300} alt={news.title} />
    <p>{news.title}</p>
